...
function Fan()  {
    ...
    const fan = () => {
        ...
        if(first_name && last_name && username && email && password) {
            ...
            axios.post("http://localhost:9002/api/sign-up/fan", user).then(res => console.log(res))
        } else {
            alert("Invalid input!")
        }
            ...
            <Paper  style={paperStyle}>
                ...
                <TextField label='Password'  name="password" value={user.password} onChange={handleChange} placeholder='Password' fullWidth required type='password'/>
                 <FormControlLabel 
                   ...
                   <Button type='submit' color='green' fullWidth variant='contained' onClick={fan}>SIGN UP</Button>
            </Paper>
        </Grid>
    );
}
